...
# Load density map
filename_density = "data/relaxation_density.bin"
...
if plot_density:
    ...
    im = ax.imshow(data_density[0].T, origin="lower", animated=True, cmap="Blues",
               vmin=data_density.min(), vmax=data_density.max())    
    ...
    ani = FuncAnimation(fig, update, frames=steps, interval=100, blit=False, repeat=True)

    plt.tight_layout()
    plt.show()
    ani.save("plots/milestone3/relaxation.gif", writer="pillow", fps=5)
