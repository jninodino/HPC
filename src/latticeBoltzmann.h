...
struct GhostBuffers {
    ...
    field_t::HostMirror send_host;
    field_t::HostMirror recv_host;

    explicit GhostBuffers(int height)
        : send_device("ghost_send_device", height * 3),
          recv_device("ghost_recv_device", height * 3),
          send_host(Kokkos::create_mirror_view(send_device)),
          recv_host(Kokkos::create_mirror_view(recv_device))
    {}
};
