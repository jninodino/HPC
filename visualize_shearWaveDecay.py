...
def analyze_slope(omega: float):
    # Load velocity field
    filename_velocity = f"data/shearWaveDecay_velocity_{omega_dic[omega]}.bin"
    ...
    print("omega:", omega)
    print("min amplitude:", np.min(a_sim))
    print("max amplitude:", np.max(a_sim))
    print("first 10:", a_sim[:10])
...
"")
plt.title("Shear-wave decay: Velocity vs. Omega")
plt.xlabel("omega")
plt.ylabel("v")
plt.legend()
plt.savefig("plots/milestone4/shearWaveDecay.png")
plt.show()
